...
<div id="Calidad" class="tabcontent">
    ...
    while ($fila=mysqli_fetch_array($res)) { 
        ...
        if($Report14 == 1 ){
            ...
            if(in_array($Rolarray[0], $separada) || in_array($Rolarray[1], $separada) || in_array($Rolarray[2], $separada) || in_array($Rolarray[3], $separada) || in_array($Rolarray[4], $separada) ){
                $contador++;
                $prueba14 = 1;
                echo "<script>pruebita('ContGraciasMetadatos');</script>";
            ?>
            ...
            <div class="request requestMetadatos">
                <?php 
                        ...
                        $ResSelect=  mysqli_query($conn,$TraerSelect);
                            ...
                            ?>
                <select class="Metadatos" name="<?php echo "Metadatos". $contador?>"
                    id="<?php echo "Metadatos". $contador?>" required>
                    ...
                    <select data-placeholder="Seleccione uno o varios" multiselect-hide-x="true" multiple
                        ...
                        id="<?php echo "Metadatos". $contador ?>">
                            ...
                            ?>
                        <textarea class="Metadatos form-control" placeholder="Si/No Justifique su respuesta"
                            style="height: 100px" id="<?php echo "Metadatos". $contador ?>"
                            name="<?php echo "Metadatos". $contador ?>"></textarea>
                        <input type="hidden" name="<?php echo "PreguntaAbiertaMetadatos". $contador ?>" value="Null"
                            class="Metadatos" id="<?php echo "PreguntaAbiertaMetadatos". $contador ?>">
                        <input type="hidden" value="<?php echo $idPregunta?>"
                            id="<?php echo "PreguntaMetadatos".$contador ?>"
                            id="<?php echo "PreguntaMetadatos".$idPregunta ?>">
                        <?php  
                            }elseif($ValTipoPregunta == 'Porcentaje'){
                            ?>
                        <input type="number" class="Metadatos" placeholder="Agregue el porcentaje"
                            id="<?php echo "Metadatos". $contador ?>" name="<?php echo "Metadatos". $contador ?>">
                        <input class="Metadatos" type="hidden"
                            name="<?php echo "PreguntaAbiertaMetadatos". $contador ?>" value="Null"
                            id="<?php echo "PreguntaAbiertaMetadatos". $contador ?>">
                        <input type="hidden" value="<?php echo $idPregunta?>"
                            id="<?php echo "PreguntaMetadatos".$contador ?>"
                            id="<?php echo "PreguntaMetadatos".$idPregunta ?>">
                        <?php 
                    ...
                    <textarea class="Metadatos form-control" placeholder="Justifique su respuesta" style="height: 100px"
                        name="<?php echo "PreguntaAbiertaMetadatos". $contador ?>"
                        id="<?php echo "PreguntaAbiertaMetadatos". $contador ?>"></textarea>
                    <input type="hidden" value="<?php echo $idPregunta?>"
                        id="<?php echo "PreguntaMetadatos".$contador ?>"
                        id="<?php echo "PreguntaMetadatos".$idPregunta ?>">
                    <?php 
    ...
    if(empty($prueba14)){
        $CambiarEstado = "UPDATE `persona` SET `repo14` = '1' WHERE idPersona = '$id'" ;
        $res2 =  mysqli_query($conn,$CambiarEstado);
        echo "<script>addClasses('btnli14');</script>";
    } 
?>
</div>
