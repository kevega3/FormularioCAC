<div class="ContGracias">
    <h3 id="ContGraciasGestionArquitectura">
        No hay preguntas
    </h3>
</div>
...
<div id="Calidad" class="tabcontent">
    ...
    while ($fila=mysqli_fetch_array($res)) { 
        ...
        if($Report17 == 1 ){
            ...
            if(in_array($Rolarray[0], $separada) || in_array($Rolarray[1], $separada) || in_array($Rolarray[2], $separada) || in_array($Rolarray[3], $separada) || in_array($Rolarray[4], $separada) ){
                $contador++;
            $prueba17 = 1;
            echo "<script>pruebita('ContGraciasGestionArquitectura');</script>"; 
            ?>
            ...
            <div class="request requestGestionArqui">
                <?php 
                        ...
                        $ResSelect=  mysqli_query($conn,$TraerSelect);
                            ...
                            ?>
                <select class="Gestion_Arquitectura" name="<?php echo "GestionArquitec". $contador?>"
                    id="<?php echo "GestionArquitec". $contador?>" required>
                    ...
                    <select data-placeholder="Seleccione uno o varios" multiselect-hide-x="true" multiple
                        class="chosen-select " name="<?php echo "GestionArquitec". $contador?>[]"
                        id="<?php echo "GestionArquitec". $contador ?>">
                        ...
                        <?php    
                            ...
                            ?>
                        <textarea class="Gestion_Arquitectura form-control" placeholder="Si/No Justifique su respuesta"
                            style="height: 100px" id="<?php echo "GestionArquitec". $contador ?>"
                            name="<?php echo  "GestionArquitec". $contador ?>"></textarea>
                        <input type="hidden" name="<?php echo "PreguntaAbiertaGestionArquitec". $contador ?>"
                            value="Null" class="Gestion_Arquitectura"
                            id="<?php echo "PreguntaAbiertaGestionArquitec". $contador ?>">
                        <input type="hidden" value="<?php echo $idPregunta?>"
                            id="<?php echo "PreguntaGestionArquitec".$contador ?>"
                            id="<?php echo "PreguntaGestionArquitec".$idPregunta ?>">
                        <?php  
                            }elseif($ValTipoPregunta == 'Porcentaje'){
                            ?>
                        <input type="number" class="Gestion_Arquitectura" placeholder="Agregue el porcentaje"
                            id="<?php echo "GestionArquitec". $contador ?>"
                            name="<?php echo "GestionArquitec". $contador ?>">
                        <input class="Gestion_Arquitectura" type="hidden"
                            name="<?php echo "PreguntaAbiertaGestionArquitec". $contador ?>" value="Null"
                            id="<?php echo "PreguntaAbiertaGestionArquitec". $contador ?>">
                        <input type="hidden" value="<?php echo $idPregunta?>"
                            id="<?php echo "PreguntaGestionArquitec".$contador ?>"
                            id="<?php echo "PreguntaGestionArquitec".$idPregunta ?>">
                    ...
                    <textarea class="Gestion_Arquitectura form-control" placeholder="Justifique su respuesta"
                        style="height: 100px" name="<?php echo "PreguntaAbiertaGestionArquitec". $contador ?>"
                        id="<?php echo "PreguntaAbiertaGestionArquitec". $contador ?>"></textarea>
                    <input type="hidden" value="<?php echo $idPregunta?>"
                        id="<?php echo "PreguntaGestionArquitec".$contador ?>"
                        id="<?php echo "PreguntaGestionArquitec".$idPregunta ?>">
                    <?php 
    ...
    if(empty($prueba17)){
        $CambiarEstado = "UPDATE `persona` SET `repo17` = '1' WHERE idPersona = '$id'" ;
        $res2 =  mysqli_query($conn,$CambiarEstado);
        echo "<script>addClasses('btnli17');</script>";
    } 
