<div class="ContGracias">  
    <h3 id="ContGraciasArquitecturaDatos">
        No hay preguntas
    </h3>
</div>
...
<div id="Calidad" class="tabcontent">
    ...
    while ($fila=mysqli_fetch_array($res)) { 
        ...
        if($Report3 == 1 ){
            ...
            if(in_array($Rolarray[0], $separada) || in_array($Rolarray[1], $separada) || in_array($Rolarray[2], $separada) || in_array($Rolarray[3], $separada) || in_array($Rolarray[4], $separada) ){
                $contador++;
                $prueba3 = 1;
                echo "<script>pruebita('ContGraciasArquitecturaDatos');</script>"; 
            ?>
    ...
    if(empty($prueba3)){
        $CambiarEstado = "UPDATE `persona` SET `repo3` = '1' WHERE idPersona = '$id'" ;
        $res2 =  mysqli_query($conn,$CambiarEstado);
        echo "<script>addClasses('btnli3');</script>";
    } 
