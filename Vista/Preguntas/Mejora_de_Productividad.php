<div class="ContGracias">  
    <h3 id="ContGraciasMejoraProductividad">
        No hay preguntas
    </h3>
</div>
...
<div id="Calidad" class="tabcontent">
    ...
    while ($fila=mysqli_fetch_array($res)) { 
        ...
        if($Report9 == 1 ){
            ...
            if(in_array($Rolarray[0], $separada) || in_array($Rolarray[1], $separada) || in_array($Rolarray[2], $separada) || in_array($Rolarray[3], $separada) || in_array($Rolarray[4], $separada) ){
                $contador++;
                $prueba9 = 1;
                echo "<script>pruebita('ContGraciasMejoraProductividad');</script>";   
            ?>
                ...
                <div class="request requestMejoraProduc">
                    ...
                    <select class="MejoraProduc" name="<?php echo "Productiv". $contador?>" id="<?php echo "Productiv". $contador?>" required>
                        ...
                        <?php    
                            ...
                            ?>
                        <select data-placeholder="Seleccione uno o varios"  multiselect-hide-x="true"  multiple class="chosen-select " name="<?php echo "Productiv". $contador?>[]"  id="<?php echo "Productiv". $contador ?>">
                                                        <?php    
                            }
    ...
    if(empty($prueba9)){
        $CambiarEstado = "UPDATE `persona` SET `repo9` = '1' WHERE idPersona = '$id'" ;
        $res2 =  mysqli_query($conn,$CambiarEstado);
        echo "<script>addClasses('btnli9');</script>";
    } 
