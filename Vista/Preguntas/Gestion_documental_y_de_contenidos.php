<div class="ContGracias">  
    <h3 id="ContGraciasGestiondocumental">
        No hay preguntas
    </h3>
</div>
    ...
    while ($fila=mysqli_fetch_array($res)) { 
        ...
        if($Report11 == 1 ){
            ...
            if(in_array($Rolarray[0], $separada) || in_array($Rolarray[1], $separada) || in_array($Rolarray[2], $separada) || in_array($Rolarray[3], $separada) || in_array($Rolarray[4], $separada) ){
                $contador++;
$prueba11 = 1;
echo "<script>pruebita('ContGraciasGestiondocumental');</script>";   
            ?>
    ...
    if(empty($prueba11)){
        $CambiarEstado = "UPDATE `persona` SET `repo11` = '1' WHERE idPersona = '$id'" ;
        $res2 =  mysqli_query($conn,$CambiarEstado);
        echo "<script>addClasses('btnli11');</script>";
    } 
